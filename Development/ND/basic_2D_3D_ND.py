...
import numpy as np
from scipy import ndimage  # Import ndimage for smoothing
...
def render_trajectory_image(image, extents, params, color_map, mode='2D', smoothing_sigma=4.0):
    if mode == '2D':
        ...
        img = ax.imshow(smoothed_image, origin='lower', cmap=color_map, extent=extents, interpolation='none')
        ...
        max_hit_count = np.max(smoothed_image)
        cbar.set_ticks(np.linspace(0, max_hit_count, num=8))
        plt.tight_layout()
        plt.show()

    elif mode == '3D':
        # Apply smoothing to the image before rendering in 3D
        smoothed_image = ndimage.gaussian_filter(image, sigma=smoothing_sigma)  # Apply Gaussian filter
        ...
        x, y = np.meshgrid(x, y)
        z = smoothed_image / np.max(smoothed_image) if np.max(smoothed_image) > 0 else smoothed_image
