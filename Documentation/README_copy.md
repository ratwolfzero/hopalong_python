  ...
  - [Features, Functionality, and Special Scenarios](#features-functionality-and-special-scenarios)
    ...
    - [Pixel-Based Density Approximation](#pixel-based-density-approximation)
      - [Comparison of Pixel-Based Density Approximation vs. Histogram-Based Density Estimation](#comparison-of-pixel-based-density-approximation-vs-histogram-based-density-estimation)
        ...
        - [Histogram-Based Estimation](#histogram-based-estimation-1)
      - [Summary](#summary)
...
- **Continuous to Discrete Mapping**  
  ...
  Mathematically, let the trajectory points be represented as \((x_i, y_i)\), where \(i = 1, 2, \dots, N\) denotes the total number of points. The continuous space is bounded by \([x_{\min}, x_{\max}]\) and \([y_{\min}, y_{\max}]\). For an image of resolution \(W \times H\) (width and height), the mapping to discrete pixel indices \((p_x, p_y)\) is:
  \[
  p_x = \left\lfloor W \cdot \frac{x_i - x_{\min}}{x_{\max} - x_{\min}} \right\rfloor, \quad
  p_y = \left\lfloor H \cdot \frac{y_i - y_{\min}}{y_{\max} - y_{\min}} \right\rfloor
  \]
  where \(\lfloor \cdot \rfloor\) denotes the floor function, or equivalently, rounding to the nearest integer.
...
- **Density Tracking**  
  ...
  **Density Heatmap Matrix**. Mathematically, this process can be described as follows:

  1. **Initialization**: Define the density heatmap matrix \(D\) as a zero-initialized 2D array of dimensions \(W \times H\):
     \[
     D[p_x, p_y] = 0, \quad \forall p_x \in [0, W-1], \; p_y \in [0, H-1].
     \]

  2. **Density Accumulation**: For each trajectory point \((x_i, y_i)\) mapped to pixel indices \((p_x, p_y)\), increment the corresponding matrix entry:
     \[
     D[p_x, p_y] \gets D[p_x, p_y] + 1.
     \]

  3. **Normalization**: To enhance interpretability, normalize the matrix \(D\) by its maximum value \(D_{\max}\):
     \[
     D_{\text{norm}}[p_x, p_y] = \frac{D[p_x, p_y]}{D_{\max}},
     \]
     where \(D_{\max} = \max_{p_x, p_y} D[p_x, p_y]\).

  After processing all \(N\) trajectory points, the matrix \(D\) encodes the total number of hits (or density) at each pixel. Pixel hit counts encode the spatial distribution of trajectory point densities in continuous space. Higher hit counts indicate greater density.  
...
This process generates a density heatmap matrix where each pixel corresponds to a specific region in continuous space, with emergent density patterns arising from the interaction between the attractor's dynamics and the mapping of continuous coordinates to discrete pixel indices. For the impact of image resolution, see above.
...
- Method Invariance (Invariant Measure)  
  Regardless of the density estimation techniques or visualization settings (resolution, bin size), the intrinsic geometric structure (shape) and true dynamics of the attractor remain unchanged. These techniques affect the density representation but not the attractor itself, which is determined by the underlying mathematical functions.
...
| Parameters                | Iterations       | PCC      | CS       | SSIM    |
|---------------------------|------------------|----------|----------|---------|
| a = 0.6, b = 0.5, c = 0   | n = 10,000       | 0.6346   | 0.6376   | 0.9633 |
| a = 0.6, b = 0.5, c = 0   | n = 100,000      | 0.7724   | 0.7843   | 0.9392  |
...
Note: The historical approach based on `Dewdney's algorithm` using intermediate variables is applied. Due to the internal optimization by the Numba JIT compiler, this approach does not incur any performance loss.
