...
def main(image_size=(1000, 1000), color_map='hot'):
    try:
        ...
        image=image/np.max(image)
        hist_density=hist_density/np.max(hist_density).astype(np.uint64)
        
        data_range = max(np.max(image), np.max(hist_density)) - min(np.min(image), np.min(hist_density))
        ssim_index = ssim(image, hist_density, data_range=1)
