...
def add_noise(matrix, noise_type='gaussian', noise_level=0.1):
    ...
    if noise_type == 'gaussian':
        ...
        uniform_noise = np.random.uniform(-noise_level * np.max(matrix), 
                                           noise_level * np.max(matrix), 
                                           size=matrix.shape)
        noisy_matrix += uniform_noise
    elif noise_type == 'salt_pepper':
        prob = noise_level / 2
        rand_matrix = np.random.rand(*matrix.shape)
        noisy_matrix[rand_matrix < prob] = 0
        noisy_matrix[rand_matrix > 1 - prob] = np.max(matrix)
...
def compute_statistics(image, hist_density):
    ...
    ssim_value = structural_similarity_index(normalized_image, normalized_hist_density)
