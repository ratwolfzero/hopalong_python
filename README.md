...
The programs leverage the `math.copysign` function, `copysign(x, y)`, which returns a float with the absolute value of x but the sign of y. On platforms that support signed zeros, `copysign(1.0, -0.0)` correctly evaluates to `-1.0`.
